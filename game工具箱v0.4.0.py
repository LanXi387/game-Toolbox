from tkinter import ttk, Frame, Toplevel, Tk
import webbrowser
import dl_steam
...
class Main_Window(Frame):
    ...
    def create_widgets(self):
        ...
        # 其余按钮
        self.create_button(
            "作者主页", 279, 216, "https://space.bilibili.com/1890708787", is_link=True
        )
        self.create_button(
            "项目仓库",
            180,
            215,
            "https://github.com/LanXi387/game-Toolbox/",
            is_link=True,
        )
        self.create_button(
            "PCL赞助", 19, 159, "https://afdian.com/a/LTCat", is_link=True
        )
        self.create_button(
            "HMCL赞助", 139, 159, "https://afdian.com/a/huanghongxun", is_link=True
        )
        self.create_button(
            "bakaXL赞助", 262, 159, "https://afdian.com/a/TT702", is_link=True
        )
    ...
    def create_button(self, text, x, y, command, is_link=False):
        button = ttk.Button(
            self.top,
            text=text,
            command=lambda: self.open_url(command) if is_link else command(),
        )
    ...
    def show_epic_tutorial(self):
        ...
        # 显示教程内容
        label = ttk.Label(
            tutorial_window, text="这是 Epic 的教程内容。", wraplength=250
        )
        label.pack(pady=20)
    ...
    def show_steam_tutorial(self):
        ...
        label = ttk.Label(
            tutorial_window,
            text="先点击下载按钮，等待下载完成后点击安装，之后请检查桌面",
            wraplength=250,
        )
        label.pack(pady=20)
    ...
    def show_hmcl_tutorial(self):
        ...
        # 显示教程内容
        label = ttk.Label(
            tutorial_window, text="这是 HMCL 的教程内容。", wraplength=250
        )
        label.pack(pady=20)
    ...
    def show_bakaxl_tutorial(self):
        ...
        # 显示教程内容
        label = ttk.Label(
            tutorial_window, text="这是 BakaXL 的教程内容。", wraplength=250
        )
        label.pack(pady=20)
