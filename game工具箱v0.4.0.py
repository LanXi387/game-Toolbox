...
#steam
    def show_steam_tutorial(self):
        ...
        # 显示教程内容
        label = ttk.Label(tutorial_window, text="先点击下载按钮，等待下载完成后点击安装，之后请检查桌面", wraplength=250)
        label.pack(pady=20)

        # 添加下载按钮
        download_button = ttk.Button(tutorial_window, text="下载", command=lambda: dl_steam.download_steam())
        download_button.pack(pady=10)

        #添加安装按钮
        install_button = ttk.Button(tutorial_window, text="安装", command=lambda: dl_steam.install_steam('SteamSetup.exe'))
...
#watt
    def show_watt_tutorial(self):
        ...
        # 添加下载按钮
        download_button = ttk.Button(tutorial_window, text="下载", command=lambda: webbrowser.open("https://steampp.net/"))
        download_button.pack(pady=10)
